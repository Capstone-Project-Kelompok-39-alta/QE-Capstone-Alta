...
@Step("Admin send GET HTTP request for get user wihout login")
public void sendHttpRequestWithoutLogin(){
    SerenityRest.given().header("Content-Type", "application/json").get(setGetApiForGetUser());
}


    @Step("Admin receive valid HTTP response code 400 for get user")
    public void receiveCode400ForGetUser(){
        restAssuredThat(response -> response.statusCode(400));
    }

    @Step("Admin receive message invalid or expired jwt for get user")
    public void receiveMsgExpiredJwt(){
        Response response = SerenityRest.lastResponse();
        String message = response.getBody().jsonPath().get("message");
